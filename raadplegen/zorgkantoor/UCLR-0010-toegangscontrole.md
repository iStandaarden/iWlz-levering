# Toegangscontrole: Raadplegen van de behandelingperiode door het zorgkantoor (UCLR-0010)  
...
- **Omschrijving:** Uitvoeren van GraphQL-query [`QLR-0010-ZK.graphql`](/gql-query/zorgkantoor/QLR-0010-ZK.graphql) op het Leveringsregister door een zorgkantoor
...
### **Context**
- **Query-parameters vereist:** Het `behandelingperiodeID` en `bemiddelingspecificatieID` moeten zijn meegegeven in de query
- **Toegangsvoorwaarde:**  Er is alleen toegang als aan alle volgende voorwaarden is voldaan:
  - De parameter `behandelingperiodeID` en `bemiddelingspecificatieID` zijn aanwezig in de query;
...
> Toegang tot het Leveringsregister via query [`QLR-0010-ZK.graphql`](/gql-query/zorgkantoor/QLR-0010-ZK.graphql) is **alleen toegestaan** als:
>
> - Parameter **`behandelingperiodeID`** **en**  **`bemiddelingspecificatieID`** zijn meegegeven in de query
...
# Toegangscontrole-flows Zorgkantoor uitvoerend: QLR-0010-ZK.graphql
...
stateDiagram
  ...
  indienen: Ontvang QLR-0010-ZK + Access token
  validerenT: Valideer access token
  validerenR: Valideer Request
  checkInput01:behandelingperiodeID en bemiddelingspecificatieID aanwezig?
...
| 5. | Het zorgkantoor krijgt toegang tot alle entiteiten (m.u.v. Verzoek en VerzoekAanbieder) die bij de leveringperiode en Levering horen waar de behandelingperiodeID uit de query bij hoort.
| 6. | *Einde*
...
---
Ga naar [UC beschrijving raadplegen](/raadplegen/zorgkantoor/UCLR-0010-raadplegen.md) -- Terug naar [Raadplegen](/raadplegen/README.md)