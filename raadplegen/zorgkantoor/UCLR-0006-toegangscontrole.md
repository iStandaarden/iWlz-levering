...
- **ID:** `leveringperiodeID`; `bemiddelingspecificatieID`
- **Beperking:** Alleen toegang tot gegevens van de Leveringsperiode (en overige informatie) waarvoor het zorgkantoor:
  1. Aan te merken is als het uitvoerend zorgkantoor voor de Bemiddelingspecificatie waaraan de Leveringsperiode via Levering is gekoppeld;
  2. Aan te merken is als een verantwoordelijk zorgkantoor, die betrokken is bij dezelfde Bemiddeling waaronder de Bemiddelingspecificatie valt waaraan de Leveringsperiode via Levering is gekoppeld. 
...
### **Context**
- **Query-parameters vereist:** Het `leveringperiodeID` en `bemiddelingspecificatieID` moeten zijn meegegeven in de query
- **Toegangsvoorwaarde:**  Er is alleen toegang als aan alle volgende voorwaarden is voldaan:
  - De parameter `leveringperiodeID` en `bemiddelingspecificatieID` zijn aanwezig in de query;
...
> Toegang tot het Leveringsregister via query [`QLR-0006-ZK.graphql`](/iWlz-levering/gql-query/zorgkantoor/QLR-0006-ZK.graphql) is **alleen toegestaan** als:
>
> - Parameter **`leveringperiodeID`** **en**  **`bemiddelingspecificatieID`** zijn meegegeven in de query
...
# Toegangscontrole-flows Zorgkantoor uitvoerend: QLR-0006-ZK.graphql
...
stateDiagram
  ...
  indienen: Ontvang QLR-0006-ZK + Access token
  validerenT: Valideer access token
  validerenR: Valideer Request
  checkInput01:leveringperiodeID en bemiddelingspecificatieID aanwezig?
...
| 5. | Het zorgkantoor krijgt toegang tot alle entiteiten (m.u.v. Verzoek en VerzoekAanbieder) die bij de Levering horen waar de leveringperiodeID uit de query bij hoort.
| 6. | *Einde*
